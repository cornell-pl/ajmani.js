...
\subsection{Optimizing edit mapping function composition}
  ...
  Permitting the presence of queries in edits, we define a generalization of $get_1$ for a ce-lens $I \leftrightarrow X$, where $I$ and $X$ are Q-modules:
\[
  fget_1 : \forall \phi. \sum_{q \in Q_\phi} (\phi \rhd q \to \delta I) \to \sum_{q \in Q_\phi} (\phi \rhd q \to \sum_{q' \in Q_I} (I \rhd q' \to \delta X))
\]

  The idea is motivated by the stream fusion paper of having an intermediate structure to optimize composition.
 \subsubsection{Laws}
...
\begin{mathpar}
  ...
  \begin{array}{lcl}
    get_0          &=& k.get_0 \circ l.get_0 \\
    fget_1~M~(q,f) &=& l.fget_1~(M, A) (q, q') (k.fget_1~M~(q,f)) \\
    put            &=& k.put \circ l.put \\
%     init       &=& k.init \circ l.init 
  \end{array}
