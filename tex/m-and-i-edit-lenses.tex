...
Given a Q-module $I$, $Q_I$ has subqueries if there is a homomorphism $f : \sum\limits_{q \in Q_I} (I \rhd q \to Q_I) \to $ as 
\begin{align*}
...
\begin{mathpar}
  ...
  \begin{array}{lcl}
    ...
    fget_1~M~(q,f) &=& \letin{\_, g}{k.fget_1~M~(q,f)} \\
                   ...
                   && \quad (q' \dcirc (fst \circ f), k_{A \to B} \circ snd \circ f)\\
                   && \quad \where{f}{\lambda v'.} \\
                   && \qquad \letin{\_, j}{l.get~I~(q', h)} \\
                   && \qquad \letin{(q'', p)}{j~v'} \\
                   && \qquad (q' \times lift_{B \to A} (q''), p) \\                   
    put            &=& k.put \circ l.put 
%     init       &=& k.init \circ l.init 
  \end{array}
