...
\item The database model consist of $DB$ module $(|DB|,E,\odot)$ where the carrier is set of instances of the database.
\item The database model consist of a Q-module $(DB,Q,\rhd,V)$.
\item $V$ is a module with carrier set as all possible tables (and database) which can be generated as a result of query on $DB$.
\item Each element $s$ in $S$ has an ideal-lens $L(s)$ associated with it. For $s_1,s_2 \in S$, $L(s_1 . s_2) ~=~ L(s_1) . L(s_2)$.
 ...
 The homset between a pair of source schemas in $\db$ is $S$, and between any other pair of objects is a subset of $Q$. Composition of morphisms respects composition in $Q$ and $S$, as well as the action $rw$. Every morphism in $\db$ is a functor.
