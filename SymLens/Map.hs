...
appendInto :: (Enum k, Ord k, Show k) => SymLens (Map.Map k a, Map.Map k a) (Map.Map k a)
appendInto = 
  SymLens Map.empty
...
insert :: (Ord k, Eq a, Show a) => k -> a -> SymLens (Map.Map k a) (Map.Map k a)
insert k initA = 
...
fmmap :: (Ord k, Show k) => SymLens a b -> SymLens (Map.Map k a) (Map.Map k b)
